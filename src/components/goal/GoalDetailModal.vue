...
const progressColor = computed(() => {
  switch (goalDetail.value?.status) {
    case 'COMPLETED': return '#22B14C'
    case 'EXCEEDED': return '#ED1C24'
    case 'FAILED': return '#9CA3AF'
  ...
  >
    ...
    >
      ...
      <div v-else-if="goalDetail" class="space-y-6">
        ...
        <!-- Status Message -->
        <div
            v-if="goalDetail.statusMessage"
            class="p-4 rounded-lg text-center font-medium"
            :class="{
    'bg-green-50 text-green-700': goalDetail.status === 'COMPLETED',
    'bg-red-50 text-red-700': goalDetail.status === 'EXCEEDED',
    'bg-gray-50 text-gray-600': goalDetail.status === 'FAILED',
    'bg-blue-50 text-blue-700': goalDetail.status === 'ACTIVE'
  }"
        >
